...
module Exits
  class MicroTP < ApplicationService
    ...
    def initialize(order:, ltp:, entry_ts:, side:)
      @order     = order
      @ltp       = ltp.to_f
      @entry_ts  = entry_ts || Time.zone.now
      @side      = side
    ...
    def chop_live?(mfe_r:, stalled:, spreadp:)
      ...
      return true if (Time.zone.now - @entry_ts) > 300 && mfe_r < 0.5
    ...
    def time_stop!
      ...
      return unless (Time.zone.now - @entry_ts) > 480
    ...
    def stalled_seconds
      ...
      last = Rails.cache.fetch(key) do
        { ltp: @ltp, ts: Time.zone.now }
      end

      if (@ltp - last[:ltp]).abs >= (@r * 0.10)
        Rails.cache.write(key, { ltp: @ltp, ts: Time.zone.now }, expires_in: 1.hour)
        0
      else
        (Time.zone.now - (last[:ts] || Time.zone.now)).to_i
      end
    end
