...
RSpec.describe Risk::Guard do
  ...
  before do
    ...
    allow(Live::TickCache).to receive(:get).and_return({ ltp: 100.0, ts: Time.zone.now })
  end
  ...
  it 'blocks on stale ticks' do
    allow(Live::TickCache).to receive(:get).and_return({ ltp: 100.0, ts: Time.zone.now - 10 })
    ok, reason = guard.allow_entry(expected_risk_rupees: 500, seg: 'NSE_FNO', sid: 123)
    expect(ok).to be false
  ...
  it 'blocks when daily loss cap would be breached' do
    ...
    allow(guard).to receive(:realized_loss_today_abs).and_return(1200.0)
  ...
  it 'allows when all gates pass' do
    allow(guard).to receive(:realized_loss_today_abs).and_return(200.0)
    ok, reason = guard.allow_entry(expected_risk_rupees: 200, seg: 'NSE_FNO', sid: 123)
