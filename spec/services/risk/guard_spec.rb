...
RSpec.describe Risk::Guard do
  ...
  it 'blocks when trading disabled' do
    ...
    ok, reason = guard.entry_allowed?(expected_risk_rupees: 500, seg: 'NSE_FNO', sid: 123)
  ...
  it 'blocks when daily loss cap would be breached' do
    ...
    ok, reason = guard.entry_allowed?(expected_risk_rupees: 400, seg: 'NSE_FNO', sid: 123)
    expect(ok).to be false
  ...
  it 'blocks when max trades reached' do
    ...
    ok, reason = guard.entry_allowed?(expected_risk_rupees: 200, seg: 'NSE_FNO', sid: 123)
    expect(ok).to be false
