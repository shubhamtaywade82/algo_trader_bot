#!/usr/bin/env ruby
# Test script for Rails API with Live DhanHQ Data
...
class LiveDataTester
  ...
  def test_all_endpoints
    ...
    # Test LLM endpoints with live data
    puts '=== LLM Endpoints (Live DhanHQ Data) ==='
  ...
  def test_endpoint(method, path, description, body = nil)
    ...
    begin
      ...
      if response.code.to_i == 200
        ...
        begin
          data = JSON.parse(response.body, symbolize_names: true)
          puts "⚠️  Service Unavailable: #{data[:error]} - #{data[:message]}"
        rescue JSON::ParserError
          puts "⚠️  Service Unavailable: #{response.body}"
        end
      else
...
if __FILE__ == $PROGRAM_NAME
  ...
  puts '5. Ensure DhanHQ credentials are valid'
  puts ''

  tester = LiveDataTester.new
